...
import models.{NormalMode, SocialMediaLoss, UserAnswers}
...
class SocialMediaLossControllerSpec extends SpecBase with MockitoSugar {
  ...
  "SocialMediaLossController" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(SocialMediaLossPage, SocialMediaLoss.values.head).success.value
      ...
      running(application) {
        val request =
          FakeRequest(POST, socialMediaLossRoute)
            .withFormUrlEncodedBody(("value", SocialMediaLoss.values.head.toString))

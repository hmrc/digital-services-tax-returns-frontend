...
class IsRepaymentBankAccountUKControllerSpec extends SpecBase with MockitoSugar {
  ...
  "IsRepaymentBankAccountUK Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, isRepaymentBankAccountUKRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[IsRepaymentBankAccountUKView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(IsRepaymentBankAccountUKPage(periodKey), true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, isRepaymentBankAccountUKRoute)

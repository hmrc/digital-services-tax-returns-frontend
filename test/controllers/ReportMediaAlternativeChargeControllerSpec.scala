...
import views.html.ReportMediaAlternativeChargeView

import scala.concurrent.Future

class ReportMediaAlternativeChargeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "ReportMediaAlternativeCharge Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, reportMediaAlternativeChargeRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ReportMediaAlternativeChargePage(periodKey), true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, reportMediaAlternativeChargeRoute)

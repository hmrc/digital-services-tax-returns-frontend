...
import views.html.ReportOnlineMarketplaceAlternativeChargeView

import scala.concurrent.Future

class ReportOnlineMarketplaceAlternativeChargeControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new ReportOnlineMarketplaceAlternativeChargeFormProvider()
  ...
  "ReportOnlineMarketplaceAlternativeCharge Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, reportOnlineMarketplaceAlternativeChargeRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        UserAnswers(userAnswersId).set(ReportOnlineMarketplaceAlternativeChargePage(periodKey), true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, reportOnlineMarketplaceAlternativeChargeRoute)

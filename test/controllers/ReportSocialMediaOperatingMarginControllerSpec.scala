...
import views.html.ReportSocialMediaOperatingMarginView
...
class ReportSocialMediaOperatingMarginControllerSpec extends SpecBase with MockitoSugar {
  ...
  "ReportSocialMediaOperatingMargin Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, reportSocialMediaOperatingMarginRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(ReportSocialMediaOperatingMarginPage(periodKey), validAnswer).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, reportSocialMediaOperatingMarginRoute)

...
class ReportSearchAlternativeChargePageSpec extends AnyFreeSpec with Matchers {
  ...
  "ReportSearchAlternativeChargePage" - {
    "cleanup" - {
      "must remove SearchEngineLossPage, ReportSearchEngineOperatingMarginPage when there is a change of the answer from 'Yes' to 'No'" in {
      ...
      "must retain ReportSearchEngineOperatingMarginPage, SearchEngineLossPage when there is a change of the answer to 'Yes'" in {
        ...
        result.get(ReportSearchEngineOperatingMarginPage(periodKey)) mustBe defined
      }
    }
  }
}

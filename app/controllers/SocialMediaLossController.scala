    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(SocialMediaLossPage(periodKey), value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(SocialMediaLossPage(periodKey), mode, updatedAnswers))
        )
  }
}

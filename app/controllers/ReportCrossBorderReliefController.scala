...
class ReportCrossBorderReliefController @Inject() (
  ...
  def onPageLoad(periodKey: String, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.get(ReportCrossBorderReliefPage(periodKey)) match {
        case None        => form
    ...
    (identify andThen getData andThen requireData).async { implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(ReportCrossBorderReliefPage(periodKey), value))
              _              <- sessionRepository.set(updatedAnswers)
            } yield Redirect(navigator.nextPage(ReportCrossBorderReliefPage(periodKey), mode, updatedAnswers))
        )
    }
}
